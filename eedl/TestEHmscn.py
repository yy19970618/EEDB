...
def TEST_MSCN_MODEL(seed, workload, params, table, queryset, labels, query_num_true, iteration):
    ...
    # 判断是否需要继续训练
    print(
        "Loading exist model: " + f"{table.version}_{workload}-{model.name()}_ep{args.epochs}_bs{args.bs}_{args.train_num // 1000}k" \
                                  f"-seed{seed}-query_num_true{query_num_true}-{iteration}.pt")
...
def Test_all(dataset_name, model_query_num_true, num_samples, hid_units):
    global query, index
    '''
     这里需要修改！！！！！！！
     '''
    ...
    train_size = 16000
    valid_size = 2000
    query_num_true = 2500  # 测试间隔
    query_ptr = 0  # 用来标记全局的下一个查询应该是第几号，比如已经使用了全部数据集的10000条，那么其值就是10000
    model_query_num_true = model_query_num_true
    data_pool = Memory()
    load_model = False
    init_state = True
    ...
    model_arg = {"bs": 512, "train_num": train_size, "num_samples": num_samples, "hid_units": hid_units, "epochs": 30}
    while query_ptr <= len(query_total) - query_num_true - valid_size:
        ...
        iteration = ((query_ptr - 32000) // model_query_num_true) + 1
        # 获取用于训练的数据
        ...
        best_metric, state = TEST_MSCN_MODEL(100,
                                             ...
                                             queries,
                                             labels,
                                             model_query_num_true,
                                             iteration)
        metric_history.append(best_metric)
    information = {"metric_history": metric_history, "model_arg": model_arg}
    with open(OUTPUT_ROOT + f"/information/{dataset_name}-{model_query_num_true}-test.pkl", "wb") as f:
