...
def train_mscn(seed, workload, params, size_limit, table, queryset, labels, query_num_true, iteration,
               load_model=False):
    best_metric = None
    ...
    save_model_file = model_path + "/" + f"{table.version}_{workload}-{model.name()}_ep{args.epochs}_bs{args.bs}_{args.train_num // 1000}k" \
                                         f"-seed{seed}-query_num_true{query_num_true}-{iteration}.pt"
    load_model_file = model_path + "/" + f"{table.version}_{workload}-{model.name()}_ep{args.epochs}_bs{args.bs}_{args.train_num // 1000}k" \
                                         f"-seed{seed}-query_num_true{query_num_true}-{iteration - 1}.pt"
    ...
    if size_limit > 0 and mscn_size > (size_limit * table.data_size_mb):
        L.info(f"Exceeds size limit {mscn_size:.2f}MB > {size_limit} x {table.data_size_mb}, do not conintue training!")
        return
    ...
    L.info(f"Model saved to {save_model_file}, best valid: {state['valid_error']}")
...
if __name__ == '__main__':
    ...
    valid_iter = iter(MyNumbers())
    '''
    这里需要修改！！！！！！！
    '''
    model_arg = {"bs": 512, "train_num": train_size, "num_samples": 10000, "hid_units": 16, "epochs": 30}

    epoch_start_str = " ----------------------------------------------epoch_start--" \
                      "------------------------------------------------------------------"

    while query_ptr <= len(query_total) - query_num_true - valid_size:
        ...
        '''
        if init_state:
            print(f"Estimating {train_size} queries...")
            for _ in range(train_size):
                sql_index = next(global_iter)
                query_ptr = sql_index + 1
                sql = query_2_sql(query_total[sql_index], table).replace("COUNT(*)", "*")
                card = db.getEsCard(sql)
                data_pool.save_store(query_total[sql_index], card)
            # 记录预处理时间
            ...
            load_model = True
            query_num = train_size if data_pool.size() == 0 else query_num_true
            for _ in range(query_num):
                ...
                # 记录预处理时间
            pre_process_time.append(time.time() - now)
            print(f"{query_num} new queries stored...")
            print("Qry time use: " + str(time.time() - now))

        '''
        =================训练数据，验证数据，测试数据的处理与分割===============================================================
        '''
        # 获取用于训练的数据
        ...
        print(f"{valid_size} size valid data set ok..")
        '''
        ========================模型配置操作==============================================================================
        '''
        # 训练模型
    ...
    with open(OUTPUT_ROOT + f"/information/{dataset_name}-{query_num_true}.pkl", "wb") as f:
